...
static void vop_release_dev(struct device *d)
{
	struct vop_device *vdev = dev_to_vop(d);
		...
		#define VCA_USE_DEVICE_RH_INSTEAD_OF_ARCHDATA
	#endif
#endif
...
#elif defined(VCA_USE_DEVICE_RH_INSTEAD_OF_ARCHDATA)
	device_rh_alloc(&vdev->dev);
	if (!vdev->dev.device_rh) {
		ret = -ENOMEM;
		goto free_vdev;
	}
	vdev->dev.device_rh->dma_ops = dma_ops;
#else
