...
nfs_to_var() {
    ...
    # if there's a "%n" in the server name, replace it with the content of /etc/nfs_server
    if strstr "$server" "%n"; then
        local nfs_server=""
        if [ -e /etc/nfs_server ]; then
            read nfs_server < /etc/nfs_server
        fi
        server=${server%%%n*}$nfs_server${server#*%n} # replace only the first %n
    fi
    # if there's a "%p" in the path, replace it with the content of /etc/nfs_path
    if strstr "$path" "%p"; then
        local location=""
        if [ -e /etc/nfs_path ]; then
            read location < /etc/nfs_path
        fi
        path=${path%%%p*}$location${path#*%p} # replace only the first %p
    fi
    ...
    # if there's a "%i" in the path, replace it with the IP
    if strstr "$path" "%i"; then
        local nodeip=""
        nodeip=$(ip -o -f inet addr show $2)
        nodeip=${nodeip%%/*}
        nodeip=${nodeip##* }
        path=${path%%%i*}$nodeip${path#*%i} # replace only the first %i
    fi

    echo "NFS boot from server: $server path: $path"
}
...
mount_nfs() {
    ...
    local nfs="" server="" path="" options="rsize=32k,wsize=32k"
    nfs_to_var "$nfsroot" $netif
    munge_nfs_options
    if [ "$nfs" = "nfs4" ]; then
    ...
    IP=`ip addr show dev eth0 |grep "inet "`
    if [ -z "$IP" ]; then
        return
    fi
    mount -t $nfs -o$options "$server:$path" "$mntdir"
    if [ $? -ne 0 ]; then
        echo "nfs_mount_error" > /sys/class/vca/vca/state
    else
        echo "nfs_mount_done" > /sys/class/vca/vca/state
    fi
